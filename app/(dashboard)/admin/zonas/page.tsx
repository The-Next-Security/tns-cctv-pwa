...
const getPriorityColor = (priority: Priority) => {
  const colors: Record<Priority, string> = {
    baja: 'bg-ds-muted text-ds-ink-muted border-ds-hairline',
  ...
  return colors[priority] || 'bg-ds-muted text-ds-ink-muted'
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-ds-ink-muted">Define y gestiona las areas del parque</p>
        </div>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="h-9 w-9 sm:h-10 sm:w-10 shrink-0 rounded-xl bg-ds-accent-faded flex items-center justify-center">
                <MapPin className="h-5 w-5 text-ds-accent" />
              </div>
              <div className="min-w-0">
                <p className="text-xl sm:text-2xl font-bold tabular-nums">{stats.total}</p>
                <p className="text-sm text-ds-ink-muted truncate">Total zonas</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-xl sm:text-2xl font-bold tabular-nums">{stats.activas}</p>
                <p className="text-sm text-ds-ink-muted truncate">Activas</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="h-9 w-9 sm:h-10 sm:w-10 shrink-0 rounded-xl bg-ds-signal-faded flex items-center justify-center">
                <MapPin className="h-5 w-5 text-ds-signal" />
              </div>
              <div className="min-w-0">
                <p className="text-xl sm:text-2xl font-bold tabular-nums">{stats.criticas}</p>
                <p className="text-sm text-ds-ink-muted truncate">Criticas</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card key={zona.id} className={cn(
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div className={cn(
                    ...
                    zona.priority === 'critica' && 'bg-ds-signal-faded',
                    zona.priority === 'alta' && 'bg-ds-accent-faded',
                    zona.priority === 'media' && 'bg-ds-surface',
                    zona.priority === 'baja' && 'bg-ds-muted',
                  )}>
                    <MapPin className={cn(
                      'h-5 w-5',
                      zona.priority === 'critica' && 'text-ds-signal',
                      zona.priority === 'alta' && 'text-ds-accent',
                      zona.priority === 'media' && 'text-ds-ink-muted',
                      zona.priority === 'baja' && 'text-ds-ink-muted',
                    )} />
                  </div>
                  <div>
                    <CardTitle className="text-lg">{zona.name}</CardTitle>
                    <p className="text-sm text-ds-ink-muted mt-1 line-clamp-2">
                      {zona.description}
                    </p>
                  </div>
                </div>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-ds-ink-muted flex items-center gap-1">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-ds-ink-muted">Prioridad</span>
